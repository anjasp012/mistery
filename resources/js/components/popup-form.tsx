...
export default function PopupForm({ popup }: PopupProps) {
    const [edit, setEdit] = useState(false);
    const [open, setOpen] = useState(false);
    const [imageDialog, setImageDialog] = useState('');
    const { data, setData, post, errors, processing } = useForm<PopupForm>({
        name: popup.name,
        file: null,
        _method: 'patch'
    });
    ...
    const submit: FormEventHandler = (e) => {
        ...
        post(route('admin.popup.update', popup.slug), {
            preserveScroll: true,
            onSuccess: () => {
                ...
                <>
                    <TableRow>
                        ...
                        <TableCell><img onClick={e => {setOpen(true)
                             setImageDialog(`/storage/${popup.file}`)} } className='w-10' src={`/storage/${popup.file}`} alt={popup.file} /></TableCell>
