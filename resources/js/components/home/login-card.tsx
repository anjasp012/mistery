...
export default function LoginCard() {
    const { playSound } = useSound(['error.wav']);
                    ...
                    <div className="flex">
                        <button disabled={processing} type="submit" className="w-[12vh] md:w-[20vh] mx-auto cursor-pointer group select-none">
                            {processing ?
                                ...
                                />
                                :
                                <img loading='lazy' src={`/storage/${themes.login_button}`} className="w-full group-hover:saturate-120 group-focus:translate-x-px group-focus:translate-y-px" alt="login-button" />
                            }
                        </button>
                    </div>
                </form>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                ...
                <DialogContent className="select-none focus:outline-none w-[80vw] sm:max-w-[60vh] p-0 border-none [&>button:first-of-type]:hidden bg-transparent shadow-none">
                    ...
                    <div className='absolute inset-0 flex flex-col justify-center'>
                        <div className='flex justify-center gap-2 items-center text-center font-utama text-xs sm:text-lg w-[80%] mx-auto'>{errors.username}</div>
                    </div>
                </DialogContent>
            </Dialog>
        </>
