...
export default function SliderForm({ slider }: SliderProps) {
    const [edit, setEdit] = useState(false);
    const [open, setOpen] = useState(false);
    const [imageDialog, setImageDialog] = useState('');
    const { data, setData, post, errors, processing } = useForm<SliderForm>({
        name: slider.name,
        file: null,
        _method: 'patch'
    });
    ...
    const submit: FormEventHandler = (e) => {
        ...
        post(route('admin.slider.update', slider.slug), {
            preserveScroll: true,
            onSuccess: () => {
                ...
                <>
                    <TableRow>
                        ...
                        <TableCell><img onClick={e => {setOpen(true)
                             setImageDialog(`/storage/${slider.file}`)} } className='w-10' src={`/storage/${slider.file}`} alt={slider.file} /></TableCell>
