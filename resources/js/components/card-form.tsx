...
export default function CardForm({ card }: CardProps) {
    ...
    const { data, setData, post, errors, processing } = useForm<CardForm>({
        name: card.name,
        file: null,
        _method: 'patch'
    });
    ...
    const submit: FormEventHandler = (e) => {
        ...
        post(route('admin.card.update', card.slug), {
            preserveScroll: true,
            onSuccess: () => {
                ...
                <>
                    <TableRow>
                        ...
                        <TableCell><img onClick={e => {setOpen(true)
                             setImageDialog(`/storage/${card.file}`)} } className='w-10' src={`/storage/${card.file}`} alt={card.file} /></TableCell>
